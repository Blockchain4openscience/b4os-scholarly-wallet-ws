...
def authenticate(auth_code):
    ...
    try:
        ...
        if not mongo.user_exists(user_data["orcid"]):
            ...
            thread_user = Thread(target=hyperledger.create_researcher, args=(user_data,))
            thread_user.start()
            user_data.pop("_id")
        else:
        ...
        thread_works = Thread(
            target=hyperledger.claim_ros, args=(user_data["orcid"], "orcid", works)
        )
        thread_works.start()
...
def get_works(user_token, orcid_id):
    api = orcid.PublicAPI(
        config.get("DEFAULT", "ORCID_CLIENT_ID"), config.get("DEFAULT", "ORCID_SECRET")
    )
    ...
    for work in summary["group"]:
        ...
        work_metadata["last-modified-date"] = work_summary["last-modified-date"][
            "value"
        ]
        work_metadata["type"] = work_summary["type"]
        work_metadata["publication-year"] = work_summary["publication-date"]["year"][
            "value"
        ]
