...
def publish_to_zenodo(author, disco):
        ...
        'metadata':
            {
                ...
                'description': disco['description'] if disco['description'] else disco['title'],
    ...
    r = requests.put(f'{zenodo_url}/{deposition_id}',
    ...
    r = requests.post(
        ...
        params={'access_token': zenodo_token})
    mongo.update_disco(author['orcid'], disco['_id'], {'doi': deposition_doi, 'zenodo_id': deposition_id, 'status': 'published'})
...
def harvest_file(user, research_object):
    ...
    if research_object['source'] == 'github':
        ...
        ro_content = github.download_zip_file(downloads_url, user['github_token']['access_token'])
        file_name += '.zip'
        with open(tmp_dir + file_name, 'wb') as f:
            f.write(ro_content)
    elif research_object['source'] == 'figshare':
        file_name += '.zip'
        figshare.download_files(research_object['ro'], user['figshare_token']['access_token'], tmp_dir + file_name)
    else:
